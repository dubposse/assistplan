...
let letzteMonatsDetailsKollegen = null;

function renderMonatsDetailsKollegenSpiegel() {
    const source = document.getElementById("monat-details-ausgabe");
    const mirror = document.getElementById("monat-details-kollegen-spiegel");
    if (!source || !mirror) return;

    const order = ["1", "2", "3", "4", "5", "6", "7", "8", "9", "0"];
    const sections = new Map(order.map(number => [number, {
        wunschfrei: "—",
        wunscharbeitstage: "—"
    }]));
    ...
    source.innerText.split("\n").map(line => line.trim()).forEach(line => {
        const match = line.match(/^Kollege\s*([0-9]+)\s*(.*)$/);
        if (!match || !sections.has(match[1])) return;
        const detail = match[2];
        if (detail.startsWith("Wunschfrei:")) {
            sections.get(match[1]).wunschfrei = detail.replace("Wunschfrei:", "").trim() || "—";
        }
        if (detail.startsWith("Wunscharbeitstage:")) {
            sections.get(match[1]).wunscharbeitstage = detail.replace("Wunscharbeitstage:", "").trim() || "—";
        }
    });

    if (letzteMonatsDetailsKollegen) {
        order.forEach(number => {
            const details = sections.get(number);
            const snapshot = letzteMonatsDetailsKollegen.get(number);
            if (snapshot && details.wunschfrei === "—") details.wunschfrei = snapshot.wunschfrei;
            if (snapshot && details.wunscharbeitstage === "—") details.wunscharbeitstage = snapshot.wunscharbeitstage;
        });
    }

    letzteMonatsDetailsKollegen = new Map(order.map(number => [number, { ...sections.get(number) }]));

    mirror.innerHTML = `<div class="monat-details-kollegen-grid">${order.map(number => {
        const details = sections.get(number);
        return `<section class="monat-details-kollegen-abschnitt">
            <strong>Kollege ${number}</strong>
            <div><span>Wunschfrei:</span> ${escapeHtml(details.wunschfrei)}</div>
            <div><span>Wunsch-Arbeitstage:</span> ${escapeHtml(details.wunscharbeitstage)}</div>
        </section>`;
    }).join("")}</div>`;
    mirror.hidden = false;
}

const monatDetailsKollegenObserver = new MutationObserver(renderMonatsDetailsKollegenSpiegel);
const monatDetailsQuelle = document.getElementById("monat-details-ausgabe");
if (monatDetailsQuelle) {
    monatDetailsKollegenObserver.observe(monatDetailsQuelle, { childList: true, characterData: true, subtree: true });
}
...
//    document.querySelector("#monat-details-input-container").style.display = "none";
    ...
    const monatDetailsAusgabe = document.getElementById("monat-details-ausgabe");
    if (monatDetailsAusgabe) monatDetailsAusgabe.style.maxHeight = "none";
    ...
    (function setupRenderAndDnD() {
        ...
        if (!document.getElementById(styleId)) {
            ...
            .ep-day.unassigned{background:#263b4b;border:1px solid #7894a8;color:#e8f6fb;box-shadow:inset 0 0 0 1px rgba(126,211,232,0.12),0 0 6px rgba(34,211,238,0.08)}
        ...
        let legendPopoverHtml = `
            ...
            <div class="ep-legend-popover" id="ep-legend-popover">
                ...
                <span style="display:inline-block;width:14px;height:14px;border:1px solid #7894a8;background:#263b4b;border-radius:3px;margin-right:4px;"></span> Unbesetzt<br>
...
document.getElementById("monat-input").addEventListener("change", function() {
    ...
    zeigeMonatDetailsBtn.addEventListener("click", function() {
        ...
        document.getElementById("monat-details-ausgabe").innerText += "Planungsfreie Tage: " + unbetreuteTage.join(", ") + "\n";
